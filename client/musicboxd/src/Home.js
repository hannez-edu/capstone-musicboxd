...
function Home() {
  ...
  const [popular, setPopular] = useState([
    ...
    TEMP_ALBUM,
    TEMP_ALBUM,
  ]); // Popular Albums: 5 albums (May grab according to some order/metric)
  const [recentAlbums, setRecentAlbums] = useState([
    ...
    TEMP_ALBUM,
    TEMP_ALBUM,
  ]); // Recently Listened By Followed (or in general) (Will need to know who the current user is for this if we do followed)
  const [latestReviewed, setLatestReviewed] = useState([
    TEMP_REVIEW,
    TEMP_REVIEW,
  ]); // Latest reviewed (~ 2 reviews should be fine)
    ...
    fetch(albumUrl)
      ...
      .then(async (data) => {
        ...
        for (let album of popularityAlbums) {
          album.popularity = allCatalogs.filter(
            (cata) => cata.albumId === album.albumId
          ).length;
        }
        setPopular(
          popularityAlbums
        ...
        for (let i = allCatalogs.length - 1; i >= 0; i--) {
          ...
          if (
            following.length === 0 ||
            following.includes(allCatalogs[i].userId) ||
            allCatalogs[i].userId === AuthService.getCurrentUserId()
          ) {
        ...
        setRecentAlbums(
          data.filter((alb) => recentListenedAlbums.has(alb.albumId))
        );
      })
      .catch(console.log);
  }, [allCatalogs]);
          ...
          setLatestReviewed(
            data
              .filter((review) => {
                return (
                  following.length === 0 ||
                  following.includes(review.userId) ||
                  review.userId === AuthService.getCurrentUserId()
                );
              })
              .slice(0, 2)
          );
            ...
            <div className="col" key={`${i}/${album.albumId}/${extraKey}`}>
              <div className="card border-0">
                <img
                  ...
                  src={
                    album === null || isEmptyString(album.artUrl)
                      ? ""
                      : album.artUrl
                  }
                  alt="Album cover"
                ...
                <div className="card-body text-center">
                  <p className="text-center word-break fs-4">
                    {album === null ? "loading title..." : album.title}
                  </p>
                  <p className="text-center word-break fs-5">
                    {album === null ? "loading artist..." : album.artist}
                  </p>
                  <Link
  ...
  return (
    <>
      ...
      {!hasFollowed && <h2 className="mt-3 text-center">Recently Listened</h2>}
      {hasFollowed && (
        <h2 className="mt-3 text-center">
          Recently Listened to By Followed Users
        </h2>
      )}

      {recentAlbums.isTemp == null && renderAlbums(recentAlbums, "recent")}

      {!hasFollowed && <h2 className="mt-3 text-center">Latest Reviews</h2>}
      {hasFollowed && (
